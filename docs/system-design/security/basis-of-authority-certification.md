...
![认证登录](https://oss.dearloc.com/github/javaguide/system-design/security/authentication-login.png)

授权：

![没有权限](https://oss.dearloc.com/github/javaguide/system-design/security/20210604161032412.png)
...
![RBAC 权限模型示意图](https://oss.dearloc.com/github/javaguide/system-design/security/design-of-authority-system/rbac.png)

**在 RBAC 中，权限与角色相关联，用户通过成为适当角色的成员而得到这些角色的权限。这就极大地简化了权限的管理。**

本系统的权限设计相关的表如下（一共 5 张表，2 张用户建立表之间的联系）：

![](https://oss.dearloc.com/2020-11/%E6%95%B0%E6%8D%AE%E5%BA%93%E8%AE%BE%E8%AE%A1-%E6%9D%83%E9%99%90.png)

通过这个权限模型，我们可以创建不同的角色并为不同的角色分配不同的权限范围（菜单）。

![](https://oss.dearloc.com/github/javaguide/books%E6%9D%83%E9%99%90%E7%AE%A1%E7%90%86%E6%A8%A1%E5%9D%97.png)

通常来说，如果系统对于权限控制要求比较严格的话，一般都会选择使用 RBAC 模型来做权限控制。

## 什么是 Cookie ? Cookie 的作用是什么?

![](https://oss.dearloc.com/github/javaguide/system-design/security/cookie-sessionId.png)
...
![](https://oss.dearloc.com/github/javaguide/system-design/security/session-cookie-authentication-process.png)
...
但是，并不是没有 `Cookie` 之后就不能用 `Session` 了，比如你可以将 `SessionID` 放在请求的 `url` 里面`https://dearloc.com/?Session_id=xxx` 。这种方案的话可行，但是安全性和用户体验感降低。当然，为了你也可以对 `SessionID` 进行一次加密之后再传入后端。
...
![](https://oss.dearloc.com/github/javaguide/system-design/security/20210615161108272.png)
...
![SSO 示意图](https://oss.dearloc.com/github/javaguide/system-design/security/sso.png)
...
![](https://oss.dearloc.com/github/javaguide/system-design/security/20210615151716340.png)
